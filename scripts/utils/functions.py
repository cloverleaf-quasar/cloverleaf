...
def makestats(statout, statdict, show=True):
    ...
    if show:
        ...
        print("")
        print(
            "obs min/max = {:.2e} / {:.2e}".format(
                statdict["obs_min"], statdict["obs_max"]
            )
        )
        print(
            "mod min/max = {:.2e} / {:.2e}".format(
                statdict["mod_min"], statdict["mod_max"]
            )
        )
        print(
            "res min/max = {:.2e} / {:.2e}".format(
                statdict["res_min"], statdict["res_max"]
            )
        )
        print("===================================")
